...
test.describe('My Info Page Tests', () => {
    ...
    test('Verify My Info page @regression @smoke', async ({ myInfoPage, users }) => {
        await myInfoPage.checkIfPersonalDetailsIsVisible()
    })

    test('Verify first name is updated successfully @regression', async ({ myInfoPage, users }) => {
        await myInfoPage.updateFirstName(users.employee.firstName)
        await myInfoPage.clickSaveButton()
    })

    test('Verify last name is updated successfully @regression', async ({ myInfoPage, users }) => {
        await myInfoPage.updateLastName(users.employee.lastName)
        await myInfoPage.clickSaveButton()

    })

    test('Verify check radio button is working @regression', async ({ myInfoPage }) => {
        await myInfoPage.checkRadioButton()
        await myInfoPage.clickSaveButton()

    })

    test('Verify that user is able to select the marital status @regression', async ({ myInfoPage, users }) => {
        await myInfoPage.selectMaritalStatus(users.employee.maritalStatus)
        await myInfoPage.clickSaveButton()
    })

    test('Veriy that user is able to select the date of birth @regression', async ({ myInfoPage, users }) => {
        await myInfoPage.selectDateOfBirth(users.employee.dateOfBirth)
        await myInfoPage.clickSaveButton()
    })

    test('verify file is uploaded successfully @regression', async ({ myInfoPage }) => {

        await myInfoPage.uploadFile('testexample.png')
        await myInfoPage.verifyUploadedFile('testexample.png')
    })

    test('Verify user is able to update all the personal details and success toaster us displayed @smoke @regression', async ({ myInfoPage, users }) => {
